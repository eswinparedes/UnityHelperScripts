...
{ 
    public class OptionTest : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            Option<int> oneOpt = 1.AsOption_SAFE();
            Option<int> twoOpt = 2.AsOption_SAFE();

            Option<int> intNone = None.Default;

            //Reduce from value when some 
            var reduceValueResult0 =
                oneOpt
                .Bind(val => (val + 4).AsOption_SAFE()) // 1 + 4 = 5
                .Reduce(-1); // 4
            ...
            var reduceValueResult1 =
                oneOpt
                .Bind(val => (val + 4).AsOption_SAFE())
                .Bind(val => (Option<int>)None.Default)
                .Reduce(-1);
            ...
            var reduceFunctionResult0 =
                oneOpt
                .Bind(val => (val + 4).AsOption_SAFE()) // 1 + 4 = 4
                .Reduce(() => -1); // 5
            ...
            var whereResult0 =
                oneOpt
                .Bind(val => (val * 3).AsOption_SAFE()) // 1 * 3 = 3
